...
if __name__ == "__main__":
    ...
    buscar_N_cluster = False
    cargar_mod_word_embedding = True
    prop_trn= 0.70
    cluster_v = [8]
    ...
    N_alimentos = 2
    N_memoria   = 2*N_steps
    prob_recomendacion_min = 0.1
    prob_recomendacion_max = 0.9

    factor_descuento = 0.40
    learning_rate = 1E-3

    
    decaimiento_exploracion = 20
    train_step = N_steps
    exploracion_min, exploracion_max = 0.0, 1.0    # Límites mínimo y máximo del factor exploración vs explotación
    decaimiento_exploracion =  N_episodes/5           # Se define el decaimiento del factor exploración vs explotación como una exponencial del numero de episodio, tal que para step = N_steps se alcance el valor mínimo definido

    N_alimentos_v        = [2]
    
    do_graphs = True
    print_alimentos = False
    ...
    if cargar_mod_word_embedding:
        ...
        w = models.KeyedVectors.load_word2vec_format('./Text_Sim/GoogleNews-vectors-negative300.bin', binary=True)
        vocabulario = w.vocab
    ...
    if buscar_N_cluster:
        ...
        cluster_v = np.arange(2,int(2.0*np.sqrt(nut_data.shape[1])))
...
##
##References
## GOOGLE WORD EMB: https://papers.nips.cc/paper/5021-distributed-representations-of-words-and-phrases-and-their-compositionality
